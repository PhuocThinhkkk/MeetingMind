...
import { getAudioById } from '@/modules/transcription/repository/server/audio-upload-operations'
import { saveSummaryByAudioId } from '@/modules/transcription/repository/server/summary-operations'
import { getTranscriptByAudioId } from '@/modules/transcription/repository/server/transcript-operations'
import { insertManyEventsByAudioId } from '@/modules/transcription/repository/server/events-operations'
import { isAudioFileStatusDone } from '@/modules/transcription/service/server/utils'
...
export async function POST(
  ...
  try {
    ...
    const llm = getLLM()
    const promptBuilder = buildMeetingSummaryPrompt(
      transcript.text,
      true,
      false
    )
    ...
    if (!parsed.summary) {
      log.info('Prompt: ', promptBuilder)
      log.info('GPT res: ', parsed)
      throw new Error(
        `${'How the heck AI didnt return summary. GPT return: ' + parsed}`
      )
    }
