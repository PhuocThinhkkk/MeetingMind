...
import { formatDate } from '@/utils/ui-format/time-format'
import {
  QALogBase,
  QALogRow,
  TranscriptRow,
...
import {
  getQaLogsByAudioId,
  insertQALogs,
} from '@/modules/transcription/repository/client/qa-log-operations'
...
export function QATab() {
  const { user } = useAuth()
  const { qaLogs, appendQaLog, setQaLogs, transcript, audioId } =
    useTranscriptionView()
  ...
  function toQALogUI(log: QALogInit): QALogUI {
    return {
      ...
      answer: log.answer,
      created_at:
        'created_at' in log ? (log.created_at as string) : now.toISOString(),
    }
  }
